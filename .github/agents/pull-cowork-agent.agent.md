...
this catalog of well-known artifacts. Each entry maps a short name to its
repo path. **This table is auto-regenerated by the `publish-to-public` agent
whenever it publishes this file — do not edit the rows by hand.**

| Type  | Short Name | Repo Path |
|-------|------------|-----------|
| Agent | `hub-closeout` | `.github/agents/hub-closeout.agent.md` |
| Agent | `hub-precall-summary` | `.github/agents/hub-precall-summary.agent.md` |
| Agent | `pull-cowork-agent` | `.github/agents/pull-cowork-agent.agent.md` |
| Skill | `hub-close-out-email` | `skills/hub-close-out-email/` |
| Skill | `teams-chat-export` | `skills/teams-chat-export/` |
| Skill | `transcribe-youtube` | `skills/transcribe-youtube/` |

Matching rules:
- Case-insensitive
- The `.agent.md` suffix is optional for agents
- For skills, match the folder name; the path resolves to `skills/<name>/`
- Partial prefix match is allowed if unambiguous
- If a short name collides between an Agent and a Skill, the user must
  specify the full repo path to disambiguate
...
2. hub-precall-summary — Hub Pre-Call Summary Agent
3. pull-cowork-agent — Pull Cowork Agent (this agent)

Skills:
4. hub-close-out-email
5. teams-chat-export
6. transcribe-youtube
```
